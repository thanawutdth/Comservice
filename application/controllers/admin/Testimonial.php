...
class Testimonial extends CI_Controller {
	...
	public function index(){
		...
		$data_view['edit']="yes";
		$data_view['testimonial']=$this->m_testimonial->get_testimonial();
		if (isset($_POST['file_path'])) {	
			$sort_order=1;
				foreach ($_POST['file_path'] as $key => $value) {
					if ($value!="__old") {
						echo "in if";
						$filename=$value;
						$id=$this->m_testimonial->generate_id();
					        $ext=explode(".", $filename);
					        $new_ext=$ext[count($ext)-1];
					        $new_filename=$id."_".time().".".$new_ext;
					        $file = './media/temp/'.$filename;
					        $newfile = './media/testimonial/'.$new_filename;
					        if (!is_dir('./media/testimonial/')) {
					        	mkdir('./media/testimonial/');
					        }
					        if (!copy($file, $newfile)) {
					        	...
					        	$this->m_testimonial->add_testimonial($data_insert);
					            @unlink("./media/temp/".$filename);
					            @unlink("./media/temp/thumbnail/".$filename);
					            $sort_order+=1;
					        }
						...
						$this->m_testimonial->update_testimonial($data_insert,$key);
						$sort_order+=1;
					}
				...
				if (isset($_POST['del_list'])) {
					foreach ($_POST['del_list'] as $key => $value) {
						$this->m_testimonial->delete_testimonial($value);
					}
				}
			...
			$this->load->view('admin/v_testimonial',$data_view);
